...
普通聊天会自动带上最近 30 条群聊作为上下文，可以在 `.env` 里用 `CHAT_CONTEXT_MESSAGES` 调整。连续 @ 机器人聊天时，还会保留最近 12 条临时对话记忆，可以用 `ASSISTANT_MEMORY_MESSAGES` 调整；这部分只存在内存，不会落盘。`搜索群聊` 是查本地聊天记录。
每个群可以单独选择人格，内置：默认、温柔助理、成熟温柔、项目经理、吐槽搭子、学习教练。切换人格后会清掉当前群的临时对话记忆，避免串味。
